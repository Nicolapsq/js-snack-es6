...
for (let i = 0; i < listaBici.length; i++) {
    const pesoBici = parseFloat(listaBici[i].peso);
    const pesoBiciMin = parseFloat(pesoMin.peso);
    if (pesoBici < pesoBiciMin) {
        pesoMin = listaBici[i];
    }
}
console.log(pesoMin);
...
// // Creo una nuova lista di oggetti modificata rispetto all'originale:
// const squadreCalcioMod = squadreCalcio.map(function(squadra){
//     return{
//         nome: squadra.nome,
//         falliSubiti: squadra.falliSubiti
//     }
// });
// console.log(squadreCalcioMod);